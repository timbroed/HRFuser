...
class RandomDrop:
    """
    Drop an input stream with probability p.
    """
    ...
    def __call__(self, results):
        ...
        Drop an input stream with probability p.
        """
        for key, p in zip(self.keys, self.p):
            if random.random() < p:
                if len(results[key].shape) == 3:
                    results[key][:, :, :] = np.zeros_like(results[key])
                elif len(results[key].shape) == 2 and key == 'gated_img':
                    results[key][:, :] = np.zeros_like(results[key])
                else:
                    raise Exception(f'Dimension do not match for key: {key}')
        return results

    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f'(keys={self.keys}, '
        repr_str += f'(probabilities={self.p}, '
        return repr_str
...
class Normalize:
    ...
    def __init__(self, mean, std, to_rgb=True, keys=None, with_mask=None, sensor_type='img'):
        ...
        if self.sensor_type not in ['img', 'lidar', 'radar', 'gated']:
    ...
    def __call__(self, results):
        ...
        if self.keys:
            for key in self.keys:
                if self.with_mask:
                    results[key][results[self.with_mask]] = mmcv.imnormalize(results[key], self.mean, self.std,
                                                    self.to_rgb)[results[self.with_mask]]
                else:
            ...
            for key in results.get('img_fields', ['img']):
                results[key] = mmcv.imnormalize(results[key], self.mean, self.std,
                                                self.to_rgb)
        results[self.sensor_type+'_norm_cfg'] = dict(
...
class RandomCrop:
    ...
    def __init__(self,
        ...
        self.thresh_in_frame = thresh_in_frame
    ...
    def _crop_data(self, results, crop_size, allow_negative_crop):
        ...
        if 'crop_factor' in results:
            results['crop_factor'] = (results['crop_factor'][0]+crop_y1, results['crop_factor'][1]+crop_x1)
        else:
            results['crop_factor'] = (crop_y1, crop_x1)

        # crop bboxes accordingly and clip to the image boundary
        for key in results.get('bbox_fields', []):
            # e.g. gt_bboxes and gt_bboxes_ignore
            if key+'_area_orig' not in results:
                pre_width = results[key][:, 2] - results[key][:, 0]
                pre_height = results[key][:, 3] - results[key][:, 1]
                results[key+'_area_orig'] = pre_width * pre_height
            ...
            post_heihgt = bboxes[:, 3] - bboxes[:, 1]
            post_area = post_width * post_heihgt
            in_frame = post_area / results[key+'_area_orig']
            valid_inds = (bboxes[:, 2] > bboxes[:, 0]) & (
                bboxes[:, 3] > bboxes[:, 1]) & (
                in_frame >= self.thresh_in_frame)
            ...
            results[key+'_area_orig'] = results[key+'_area_orig'][valid_inds]
            # label fields. e.g. gt_labels and gt_labels_ignore
            label_key = self.bbox2label.get(key)
            if label_key in results:
                results[label_key] = results[label_key][valid_inds]
...
class Crop:
    """
    Crop the image & bboxes & masks.
    Adapted from RandomCrop.
    """
    ...
    def __repr__(self):
        ...
        repr_str += f'offsets={self.offsets}, '
        repr_str += f'thresh_in_frame={self.thresh_in_frame})'
        return repr_str
