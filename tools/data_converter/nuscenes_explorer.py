...
class NuScenesExplorer:
    ...
    def map_pointcloud_to_image(self,
                                ...
                                return_3d_points = False,
                                with_img = True,
                                return_radar = False) -> Tuple:
        """
        ...
        mask = np.logical_and(mask, points[0, :] > 1.25)
        mask = np.logical_and(mask, points[0, :] < 1600 - 1.25)# im.size[0] -1
        mask = np.logical_and(mask, points[1, :] > 1.25)
        mask = np.logical_and(mask, points[1, :] < 900 - 1.25) # im.size[0] -1
        points = points[:, mask]
        coloring = coloring[mask]

        if return_radar:            
            return points, im, pc.points[:, mask], radar_xyz_endpoint[:, mask]
        elif return_3d_points:            
            return points, im, pc.points[:, mask]
        else:
