from SeeingThroughFog.tools.ProjectionTools.Gated2RGB.lib.warp_gatedimage import WarpingClass
from SeeingThroughFog.tools.ProjectionTools.Gated2RGB.lib.data_loader import load_vehicle_speed, load_time, load_stearing_ange
from SeeingThroughFog.tools.Raw2LUTImages.conversion_lib.process import Rectify_image
from SeeingThroughFog.tools.CreateTFRecords.generic_tf_tools.resize import resize
from SeeingThroughFog.tools.ProjectionTools.Gated2RGB.lib.image_transformer import disparity2depth_psm
    ...
    def read_data_and_process(self, entry_id, vehicle_speed, delay, angle):
        ...
        for folder in self.gated_keys:
            ...
            if not os.path.exists(file_path):
                raise Exception(f'Gated image is missing: {file_path}')
            if self.DEBUG==True:
...
if __name__ == '__main__':
    ...
    for key in tqdm.tqdm(seleced_files):
